//Poczatek konstruktora
    /**
     * Creates a new isntance of a Smart Table 
     * 
     * @constructor 
     * @param {nodeElement} element     An element to which a SmartTable new instance is appended
     * @param {object}      columns     An object with column names and data types as properties: [{columnName: 'ID', dataType: 'number'{columnName: 'First Name', dataType: 'string',} ect.];
     * @param {array}       data        Array of objects, where each object is a single data row
     * @param {object}      parameters  Additional parameteres enabling/disabling some of the methods (pagination,rows coloring,tooltips): { nextPreviousButtons: boolean, rowsPerPage: number, rowsColoring: boolean,toolTips: boolean};
     */
     ...
     */
        ...
        /** @property {object}          this.columns        An object with a function's parameter 'columns' */
        ...
        /** @property {nodeList|null}    this.allRows        List of all existing rows in the table, equalls null at initial conditions */
        this.allRows = null;
        ...
        /** @property {array}            this.arrayOfId      An array for values of records Id for further sequential numbering, empty at initial conditions  */
        this.arrayOfId = [];
        ...
        this.pagesArray = [];
        /** @property {number|null}           this.paginationRefreshNumber A parameter for pagination function which means 'stay at a currently displayed page' after inserting a new row or removing/editing existing one */
        this.paginationRefreshNumber = null;
        /** @property {number|null}           this.currentPageNumber A number of a currently displayed page to be recovered after after inserting a new row or removing/editing existing one */
        ...
        for (var key in this.data[0]) {
            ...
            if (parsIntToString.indexOf('NaN') > -1 && key.indexOf('age') === -1) {
                ...
                if (wartoscKlucza.indexOf('@') > 0) {
                    ...
                    this.dataTypeArray.push('string');
                }
     ...
     * @private
     */
    SmartTable.prototype.defineArrayInputs = function () {
    ...
    * @private
     */
    SmartTable.prototype.addFilterRow = function () {
    ...
    //Metoda 6: Dodawanie wierszy z danymi________________Metoda 6: ADD DATA ROW
    /**
     ...
     * @private
     */
    SmartTable.prototype.dataSorting = function () {
        var arraySorted = this.arrayOfId.sort(function (a, b) {
            return a - b;
        })
    };
    //Metoda 8.2c
     ...
     * @private
     */
    SmartTable.prototype.addNextNumberToInputWithID = function () {
        ...
        this.newInsertRow.children[IDindex].querySelector('input').value = nextId;
    };
    //Koniec Metoda 8.2d
    ...
    SmartTable.prototype.rowValidator = function (row) {
        ...
        if (emailValidator === true){
            ...
            /**
             * A local variable for checking if any of the input is not empty,in that case it is true 
             * @type {boolean|null} */
     ...
     * @private
     */
    SmartTable.prototype.appendNewRowtoTable = function () {
        this.table.appendChild(this.newInsertRow);
    };
    //Metoda 13
    ...
    /**
     * Adds event listener on the Edit button with a row editing callback function 
    ...
    /**
     * Adds event listener on the Remove button with a row editing callback function
     ...
     * Uses 'switch' function to hide the row if it doesn't meet the criterion of a sign and a number
    ...
    SmartTable.prototype.mathFunctionsForFiltering = function (inputValue,tdTextToNumber,itNumber) {
        /**
         * A variable to contain a comparison sign part of the inputValue, null at initial conditions
         * @type {string|null} */
    ...
    /**
     * Displays rows according to their number per page specified by user in option parametres, triggered externally by a user
     * @public
     */
    SmartTable.prototype.pagination = function () {
        // 1. Warunki poczatkowe, zerujemy i usuwamy poprzedni element z buttonami
        this.pagesArray = [];
        ...
        // 2. Dodajemy element na umieszczenie buttons
        ...
        // 3.  Zaladowanie wszystkich rows do tablicy
        /**
        ...
        // 4. Dzielenie tabeli na slicy wg stron 
        this.tableIntoSlices(rowsArray, divPaginButton)
        // 5. Zapiecie eventow na przyckiski stron
        ...
        // 6. Dodanie Przycisków next i previous wraz z dopietymi eventami
        ...
        this.firstOrCurrentPageDisplay();
    ...
    SmartTable.prototype.onClickShowChosenPageHandler = function (event) {
        ...
        this.currentPageNumberSetting(currentPageNumber);
    ...
    //Metoda 17.4
     ...
     /**
      * Adds pagination buttons Next and Previous if specified by user in the options parameter 
    ...
    /**
     * Displays: 1) the first page when pagination is refreshed after filtering, 2) the same page after operations: accepting new row, editing and removing a row
     * @private
     */
    SmartTable.prototype.firstOrCurrentPageDisplay = function () {
        if (this.paginationRefreshNumber === null) {
        ...
        if (this.paginationRefreshNumber > 0){
            ...
            if (this.currentPageNumber > this.pagesArray.length){
                currentPageNumber = this.pagesArray.length;
            } else if (this.currentPageNumber === null){
                currentPageNumber = 1;
            } else {
                currentPageNumber = this.currentPageNumber;
            };
            this.pagesArray[currentPageNumber-1].forEach(function (arraySlice) {
                arraySlice.style.display = '';
            });
            var buttonFirstPage = document.querySelectorAll('button.buttonPage')[currentPageNumber-1];
            buttonFirstPage.style.backgroundColor = 'white';
            buttonFirstPage.style.border = '3px solid black';
            buttonFirstPage.style.fontWeight = 'bold';
            this.paginationRefreshNumber = null;
    ...
    // Metoda 17.7 Okreslenie numeru biezaco wyswietlanej strony_______Metoda 19: CURRENT PAGE NUMBER
    /**
     * Defines global this.currentPageNumber according to delivered parameter
     * @param {number} currentPage A currently displayed page based on other methods
     * @private
     */
    SmartTable.prototype.currentPageNumberSetting = function (currentPage) {
        var currentPageNo = currentPage;
        if (currentPageNo === 0){
            currentPageNo = 1;
        };
        if (currentPageNo > this.pagesArray.length){
            currentPageNo = this.pagesArray.length;
        };
